            ...
            <div className="max-w-7xl mx-auto flex items-center gap-2 md:gap-8">
                ...
                <div className="flex-shrink-0">
                    <Link href="/dashboard">
                        <h1 className="text-lg md:text-2xl font-black italic tracking-tighter text-white cursor-pointer">
                            STASH<span className="text-emerald-500 md:inline hidden">.</span>
                        </h1>
                    </Link>
                </div>
                ...
                <div className="flex-1 relative group min-w-0">
                    ...
                    <input
                        ...
                        placeholder="Search rare stashes..."
                        className="w-full bg-white border-none rounded-full py-1.5 md:py-2.5 pl-9 md:pl-11 pr-4 text-[12px] md:text-sm text-slate-900 outline-none font-medium"
                    />
                </div>
                ...
                <div className="flex items-center gap-1.5 md:gap-4 flex-shrink-0">
                    ...
                    {/* NEW: LIST ITEM BUTTON (Only for Authenticated Users) */}
                    {hasMounted && isAuthenticated && (
                        <Link href="/dashboard/sell">
                            <button className="hidden sm:flex items-center gap-2 bg-emerald-500 hover:bg-white text-[#0a192f] px-3 md:px-4 py-1.5 md:py-2 rounded-full transition-all duration-300">
                                <svg className="w-3.5 h-3.5 md:w-4 md:h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M12 4v16m8-8H4" />
                                </svg>
                                <span className="text-[10px] md:text-xs font-black uppercase tracking-widest">List Item</span>
                            </button>
                        </Link>
                    )}
                    ...
                    <Link href="/dashboard/cart" className="relative p-1.5 md:p-2 hover:bg-white/10 rounded-full transition-colors group">
                        ...
                        {hasMounted && totalItems > 0 && (
                            <span className="absolute top-0.5 right-0.5 md:top-1 md:right-1 bg-emerald-500 text-[#0a192f] text-[7px] md:text-[8px] font-black w-3.5 h-3.5 md:w-4 md:h-4 flex items-center justify-center rounded-full border border-[#0a192f]">
                                {totalItems}
                            </span>
                        )}
                    </Link>
                    ...
                    {!hasMounted ? (
                        ...
                        <div className="flex items-center gap-2 md:gap-4">
                            <button onClick={handleLogout} className="flex items-center gap-1 px-1.5 md:px-3 py-1.5 md:py-2 hover:bg-red-500/10 rounded-lg group transition-all">
                                ...
                                <span className="hidden lg:block text-[9px] md:text-xs font-black uppercase tracking-widest text-white group-hover:text-red-400">Exit</span>
                            </button>
                        ...
                        <Link href="/login" className="bg-emerald-500 text-[#0a192f] px-4 py-2 rounded-lg text-[10px] md:text-xs font-black uppercase tracking-widest hover:bg-white transition-all">
                            Login
                        </Link>
                    )}
                </div>
